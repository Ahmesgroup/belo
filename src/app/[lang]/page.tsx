...
//
// BUG FIX: replaced self-HTTP fetch (process.env.NEXT_PUBLIC_APP_URL
// is undefined in Vercel server-side context → 0 results) with a
// direct Prisma query. No network round-trip, no env var dependency.
// ============================================================
...
// ISR: regenerate at most once every 2 minutes
...
// ── Direct Prisma query — no HTTP, works everywhere ──────────────────────────
      ...
      <main style={{ paddingTop: 56 }}>
        ...
        <section style={{ padding: "80px 5vw 60px", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ position: "relative", zIndex: 1, maxWidth: 720, width: "100%" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(34,211,138,.08)", border: "1px solid rgba(34,211,138,.18)", color: "var(--g2)", fontSize: 11, fontWeight: 600, padding: "5px 14px", borderRadius: 99, marginBottom: 24, letterSpacing: ".06em" }}>
              ...
              ✦ {lang === "fr" ? "Disponible à Dakar, Paris, Bruxelles" : "Available in Dakar, Paris, Brussels"}
            </div>
            ...
            <p style={{ fontSize: 16, color: "var(--text2)", lineHeight: 1.7, maxWidth: 520, margin: "0 auto 32px" }}>{t("hero_desc")}</p>
            <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap", marginBottom: 32 }}>
              <Link href={`/${lang}/salons`} style={{ padding: "13px 28px", borderRadius: 12, background: "var(--g)", color: "#fff", fontFamily: "var(--serif)", fontSize: 15, fontWeight: 700, textDecoration: "none" }}>{t("cta_find")}</Link>
              <Link href="/pour-les-salons" style={{ padding: "13px 24px", borderRadius: 12, background: "rgba(255,255,255,.06)", color: "var(--text)", border: "1px solid var(--border2)", fontSize: 14, textDecoration: "none" }}>{t("cta_salons")}</Link>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", border: "1px solid var(--border)", borderRadius: 14, overflow: "hidden", maxWidth: 600, width: "100%", margin: "0 auto" }}>
              {[
                [total > 0 ? `${Math.round(total * 50)}+` : "500+", lang === "fr" ? "Réservations" : "Bookings"],
                [total > 0 ? `${total}+` : "20+", lang === "fr" ? "Salons partenaires" : "Partner salons"],
                ["4.8★", lang === "fr" ? "Note moyenne" : "Avg. rating"],
                ["Wave ✓", "+ Orange Money"],
              ].map(([n, l], idx) => (
                <div key={idx} style={{ flex: 1, minWidth: 100, padding: "16px 12px", textAlign: "center", borderRight: idx < 3 ? "1px solid var(--border)" : "none" }}>
                  <div style={{ fontFamily: "var(--serif)", fontSize: 20, fontWeight: 700, color: "var(--g2)", marginBottom: 3 }}>{n}</div>
        ...
        <section id="how" style={{ padding: "60px 5vw", background: "rgba(11,15,22,.4)", borderTop: "1px solid var(--border)" }}>
          <div style={{ maxWidth: 1000, margin: "0 auto", textAlign: "center" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 20 }}>
              {([["🔍", t("step1_title"), t("step1_desc"), t("step1_time")], ["🕐", t("step2_title"), t("step2_desc"), t("step2_time")], ["💳", t("step3_title"), t("step3_desc"), t("step3_time")], ["✅", t("step4_title"), t("step4_desc"), t("step4_time")]] as [string, string, string, string][]).map(([icon, title, desc, time]) => (
        ...
        <section style={{ padding: "60px 5vw", textAlign: "center" }}>
          <div style={{ maxWidth: 600, margin: "0 auto", background: "var(--card)", border: "1px solid var(--border2)", borderRadius: 24, padding: "48px 32px" }}>
            ...
            <p style={{ color: "var(--text2)", marginBottom: 28 }}>{total > 0 ? total : "…"} {lang === "fr" ? "salons disponibles" : "salons available"}.</p>
        ...
        <footer style={{ padding: "32px 5vw", borderTop: "1px solid var(--border)", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", gap: 20, justifyContent: "center", flexWrap: "wrap", marginBottom: 12 }}>
            {[[t("about"), "/"], [lang === "fr" ? "Salons" : "Salons", `/${lang}/salons`], [lang === "fr" ? "Pour les gérants" : "For salons", "/pour-les-salons"], ["Plans", "/plans"], [t("privacy"), "/confidentialite"]].map(([label, href]) => (
          ...
          <p style={{ fontSize: 11, color: "var(--text3)" }}>© 2026 Belo · Dakar · Paris · Bruxelles — {t("footer_tagline")}</p>
        </footer>
      </main>
