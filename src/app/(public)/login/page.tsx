      ...
      <main style={{paddingTop:56,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:"80px 20px"}}>
        <div style={{width:"100%",maxWidth:400}}>
          <div style={{textAlign:"center",marginBottom:32}}>
            ...
            <p style={{color:"var(--text2)",fontSize:14,marginTop:8}}>{step === "phone" ? t("login_title") : t("code_title")}</p>
          </div>
          <div style={{background:"var(--card)",border:"1px solid var(--border)",borderRadius:20,padding:28}}>
            {step === "phone" ? (
              <>
                ...
                <button onClick={sendOtp} disabled={loading} style={{width:"100%",padding:14,borderRadius:12,border:"none",background:"var(--g)",color:"#fff",fontFamily:"var(--serif)",fontSize:15,fontWeight:700,cursor:"pointer"}}>{loading ? "Envoi..." : t("send_code")}</button>
              </>
            ) : (
              <>
                ...
                {error && <div style={{color:"var(--red)",fontSize:12,marginBottom:12}}>{error}</div>}
                <button onClick={verifyOtp} disabled={loading||otp.length!==6} style={{width:"100%",padding:14,borderRadius:12,border:"none",background:otp.length===6?"var(--g)":"var(--border)",color:"#fff",fontFamily:"var(--serif)",fontSize:15,fontWeight:700,cursor:"pointer"}}>{loading ? "Verification..." : t("connect")}</button>
                <button onClick={sendOtp} style={{width:"100%",marginTop:10,padding:10,borderRadius:10,border:"1px solid var(--border2)",background:"transparent",color:"var(--text3)",fontSize:12,cursor:"pointer"}}>{t("resend")}</button>
              </>
            )}
          </div>
          <div style={{textAlign:"center",marginTop:20}}><Link href="/" style={{fontSize:12,color:"var(--text3)",textDecoration:"none"}}>{t("back_home")}</Link></div>
        </div>
      </main>
    </>
  );
